...
//
//
// $Id: CPhysicsList.cc,v 1.2 2011/01/12 06:15:27 bulgarelli Exp $
// GEANT4 tag $Name:  $
//
/////////////////////////////////////////////////////////////////////////
//
...
//
////////////////////////////////////////////////////////////////////////
// /usr/local/geant4/examples/extended/hadronic/Hadr01
...
#include "G4IonPhysics.hh"
//AB93 #include "G4HadronProcessStore.hh"
...
{
    G4cout << "CPhysicsList::CPhysicsList() " << G4endl;
...
//   pMessenger = new CPhysicsListMessenger(this);

    // Particles
    particleList = new G4DecayPhysics("decays");

    // EM physics
    emCPhysicsList = new G4EmStandardPhysics();
}
...
{
    AddTransportation();
    emCPhysicsList->ConstructProcess();
    particleList->ConstructProcess();
    for(size_t i=0; i<hadronPhys.size(); i++) {
        hadronPhys[i]->ConstructProcess();
    }

    //if(dump) G4HadronProcessStore::Instance()->Dump(1); //AB93
}

//....oooOO0OOooo........oooOO0OOooo........oooOO0OOooo........oooOO0OOooo.....

void CPhysicsList::AddPhysicsList(const G4String& name)
{
    if (verboseLevel>0)
        G4cout << "CPhysicsList::AddPhysicsList: <" << name << ">" << G4endl;

    if (name == "emstandard_opt2") {

        delete emCPhysicsList;
        emCPhysicsList = new G4EmStandardPhysics_option2();

    } else if (name == "emstandard_opt1") {

        delete emCPhysicsList;
        emCPhysicsList = new G4EmStandardPhysics_option1();
        ...
        G4cout << "CPhysicsList::AddPhysicsList: <" << name << ">"
               << " is not defined"
               << G4endl;
    }
}
...
void CPhysicsList::SetBuilderList0(G4bool flagHP)
{
...
void CPhysicsList::SetBuilderList1(G4bool flagHP)
{
    hadronPhys.push_back( new G4EmExtraPhysics("extra EM"));
    hadronPhys.push_back( new G4HadronElasticPhysics("elastic",verboseLevel,
                          flagHP));
...
void CPhysicsList::SetBuilderList2(G4bool flagHP)
{
    hadronPhys.push_back( new G4EmExtraPhysics("extra EM"));
...
void CPhysicsList::SetBuilderList3(G4bool flagHP)
{
    hadronPhys.push_back( new G4EmExtraPhysics("extra EM"));
    hadronPhys.push_back( new G4HadronElasticPhysics("LElastic",verboseLevel,
                          flagHP));
...
void CPhysicsList::SetBuilderList4(G4bool)
{
    hadronPhys.push_back( new G4EmExtraPhysics("extra EM"));
...
void CPhysicsList::SetBuilderList5(G4bool flagHP)
{
    hadronPhys.push_back( new G4EmExtraPhysics("extra EM"));
...
void CPhysicsList::SetBuilderList6(G4bool flagHP)
{
    hadronPhys.push_back( new G4EmExtraPhysics("extra EM"));
...
void CPhysicsList::SetCutForGamma(G4double cut)
{
    cutForGamma = cut;
...
void CPhysicsList::SetCutForElectron(G4double cut)
{
    cutForElectron = cut;
...
void CPhysicsList::SetCutForPositron(G4double cut)
{
    cutForPositron = cut;
    SetParticleCuts(cutForPositron, G4Positron::Positron());
}

void CPhysicsList::List()
{
    G4cout << "### CPhysicsLists available: FTFC FTFP FTFP_BERT FTFP_EMV LHEP LHEP_BERT LHEP_EMV "
           << G4endl;
